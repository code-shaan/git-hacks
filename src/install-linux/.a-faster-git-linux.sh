# This bash script supports faster completion
# of Git commands through shortcuts.
#
# Copyright (C) 2017 Shantanu Sirsamkar <shantanu.us21@gmail.com>
...
#
#    1) Follow the instructions outlined in install-linux.txt
#
...
Checks out given branch\t\t\t\tgl smp-7.0
Creates and checks out new given branch\t\tgl -b smp-1234_7.0\n\n"; 1>&2;
}
...
if [ $CMD_EXE -eq 1 ]; then
  branch=$(gs | grep -oP "(?<='origin/).*?(?=')");
  $CMD_GIT $CMD_OPTS
  git branch --set-upstream-to=origin/$branch
fi
}
alias gco=_func_git_checkout
